...
MARSAdaptedSmooth <- function(data, nX, knots, y) {
  ...
  MARSAdaptedSmooth <- list(knots = knots,
                               cubic_knots = Cknots,
                               alpha = alpha)
...
#'
#' @description This function generates two new cubic basis functions from a variable and a knot previously created during MARS algorithm.
#'
#' @param data \code{data.frame} or \code{matrix} containing the variables in the model.
