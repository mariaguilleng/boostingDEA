...
#'
#' @param N Sample size.
#' @param nX Number of inputs. Possible values: \code{1}, \code{2}, \code{3}, \code{4},\code{5},\code{6}, \code{9}, \code{12} and \code{15}.
#'
#' @importFrom dplyr %>%
...
CobbDouglas <- function(N, nX) {
  ...
  if (nX == 1){
    ...
    y <- (data[, "x1"] ** 0.3) * (data[, "x2"] ** 0.1) * (data[, "x3"] ** 0.08) * (data[, "x4"] ** 0.02)
    data[, "y"] <- y * exp(-u)
    data[, "yD"] <- y
